...
export default function App() {
  ...
  function handleAddWatched(movie) {
    ...
    // it will store the my for always not delete works here
...
function NavBar({ children }) {
  return (
    <nav className='grid grid-cols-3 mx-0 items-center h-18 px-0 py-8  bg-green-500 rounded-md'>
      <Logo />
      {children}
    </nav>
  );
...
function Search({ query, setQuery }) {
  ...
  //  useEffect(function() {
  //   function callback(e){
  //  if(document.activeElement===elFocus.current) return;
  //     if(e.code==='Enter'){
  //   elFocus.current.focus()
  // setQuery('')
  //     }
          ...
          <section>
            <div className='rating'>
              {!isWatched ? (
                ...
                >
                  You Already Rated Movie with {watchedUserRating}⭐️
                  Rating
                </p>
              )}
            </div>
